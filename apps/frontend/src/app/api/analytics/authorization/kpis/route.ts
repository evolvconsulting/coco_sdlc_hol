  ...
  try {
    ...
    const sql = `
      SELECT
        ...
        SUM(CASE WHEN is_retry_attempt THEN 1 ELSE 0 END) as retry_attempts,
        ROUND(SUM(retry_recovered_count) * 100.0 / NULLIF(SUM(CASE WHEN is_retry_attempt THEN 1 ELSE 0 END), 0), 2) as retry_success_rate
